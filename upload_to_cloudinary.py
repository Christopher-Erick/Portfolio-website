#!/usr/bin/env python
"""
Script to upload local media files to Cloudinary
"""
import os
import sys
import django
from pathlib import Path

# Add the project directory to Python path
project_dir = Path(__file__).resolve().parent
os.chdir(project_dir)

# Load environment variables from .env file
def load_env_file():
    """Load environment variables from .env file if it exists"""
    env_file = project_dir / '.env'
    if env_file.exists():
        with open(env_file, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith('#') and '=' in line:
                    key, value = line.split('=', 1)
                    os.environ[key.strip()] = value.strip()

# Load environment variables
load_env_file()

# Set up Django
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'portfolio_site.settings')
django.setup()

def upload_files_to_cloudinary():
    """Upload local media files to Cloudinary"""
    try:
        import cloudinary
        import cloudinary.uploader
        
        # Get Cloudinary configuration
        cloud_name = os.environ.get('CLOUDINARY_CLOUD_NAME')
        api_key = os.environ.get('CLOUDINARY_API_KEY')
        api_secret = os.environ.get('CLOUDINARY_API_SECRET')
        
        if not (cloud_name and api_key and api_secret):
            print("Missing Cloudinary credentials")
            return
        
        # Configure Cloudinary
        cloudinary.config(
            cloud_name=cloud_name,
            api_key=api_key,
            api_secret=api_secret
        )
        
        # Upload project images
        projects_dir = project_dir / 'media' / 'projects'
        if projects_dir.exists():
            print("Uploading project images...")
            for file_path in projects_dir.iterdir():
                if file_path.is_file() and file_path.suffix.lower() in ['.png', '.jpg', '.jpeg', '.gif']:
                    print(f"Uploading {file_path.name}...")
                    try:
                        result = cloudinary.uploader.upload(
                            str(file_path),
                            folder="writeupsimages",
                            public_id=file_path.stem,
                            overwrite=True
                        )
                        print(f"  Uploaded successfully: {result['secure_url']}")
                    except Exception as e:
                        print(f"  Failed to upload {file_path.name}: {e}")
        
        # Upload writeup documents
        writeups_dir = projects_dir / 'writeups'
        if writeups_dir.exists():
            print("\nUploading writeup documents...")
            for file_path in writeups_dir.iterdir():
                if file_path.is_file() and file_path.suffix.lower() in ['.pdf', '.doc', '.docx']:
                    print(f"Uploading {file_path.name}...")
                    try:
                        result = cloudinary.uploader.upload(
                            str(file_path),
                            folder="writeups",
                            public_id=file_path.stem,
                            overwrite=True
                        )
                        print(f"  Uploaded successfully: {result['secure_url']}")
                    except Exception as e:
                        print(f"  Failed to upload {file_path.name}: {e}")
        
        print("\nUpload process completed!")
        
    except Exception as e:
        print(f"Error uploading files to Cloudinary: {e}")

if __name__ == '__main__':
    upload_files_to_cloudinary()